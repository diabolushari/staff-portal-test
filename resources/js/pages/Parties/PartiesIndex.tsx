...
// Simple pill component for statuses
...
export default function PartiesIndex({ parties }: Props) {
	...
	const handleDeleteClick = async (row: any) => {
		if (
			!confirm(
				`Delete party "${row.name ?? row.party_code}"? This action cannot be undone.`,
			)
		)
			return;
		try {
	...
	// Normalize status: prefer item.status.parameter_value; else fallback from id map
	const normalizeStatus = (item: any): string => {
		const byPayload = item?.status?.parameter_value;
		if (byPayload) return byPayload;
		const map: Record<number, string> = {
			1: "Active", // fallback assumption
			2: "Blacklisted",
		};
	...
	const rows = useMemo(() => {
		const list = Array.isArray(parties?.data) ? parties.data : [];

		// Augment with derived fields for easier filtering/sorting
		const augmented = list.map((item: any) => {
			const status_text = normalizeStatus(item);
			const email = item?.email_address ?? "";
			const phone = item?.mobile_number ?? item?.telephone_number ?? "";
			const effStart = item?.effective_start
				? new Date(item.effective_start)
				: null;
			const effEnd = item?.effective_end ? new Date(item.effective_end) : null;
			return { ...item, status_text, email, phone, effStart, effEnd };
		});

		// Filter by query (name, code, email, phone)
		const q = query.trim().toLowerCase();
		let filtered = augmented.filter((i: any) => {
			if (!q) return true;
			return (
				String(i.party_id ?? "")
					.toLowerCase()
		...
		if (statusFilter !== "all") {
			filtered = filtered.filter(
				(i: any) => i.status_text.toLowerCase() === statusFilter.toLowerCase(),
			);
		}

		// Filter by current flag
		if (currentFilter !== "all") {
			const want = currentFilter === "current";
			filtered = filtered.filter((i: any) => Boolean(i.is_current) === want);
		}
		...
		filtered.sort((a: any, b: any) => {
			...
			switch (sortKey) {
				case "party_id":
					return (Number(a.party_id) - Number(b.party_id)) * dir;
				case "party_code":
				...
				case "status_text":
					return (
						String(a.status_text).localeCompare(String(b.status_text)) * dir
					);
				case "is_current":
					return (
						(a.is_current === b.is_current ? 0 : a.is_current ? 1 : -1) * dir
					);
				...
				<div className="flex flex-col gap-3 sm:flex-row sm:items-center">
					<div className="flex-1">
						<input
							...
							className="w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm outline-none ring-0 focus:border-slate-400"
							value={query}
				...
				<Card>
					<CustomTable
						columns={[
							...
							headerButton("Current", "is_current"),
					...
					>
						{rows.map((item: any, index: number) => {
							...
							return (
								<TableRow key={item.version_id} className="hover:bg-slate-50">
									...
									<TableCell
										...
										title={String(
											item.mobile_number ?? item.telephone_number ?? "",
										)}
									>
										{item.mobile_number ?? item.telephone_number ?? "-"}
									</TableCell>
