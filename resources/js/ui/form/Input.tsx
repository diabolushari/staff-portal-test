...
export default function Input({
    ...
    return (
        <>
            {label != null && <label className="small-1stop mb-1 tracking-normal text-gray-800">{label}</label>}
            <input
                type={type}
                value={value}
                onKeyDown={handleKeydown}
                onChange={(event) => (formatter ? setValue(formatter(event.target.value)) : setValue(event.target.value))}
